...
-- 4. Members 테이블에 외래키 제약조건 추가
ALTER TABLE members 
ADD CONSTRAINT IF NOT EXISTS fk_members_position 
    FOREIGN KEY (position_id) REFERENCES positions(position_id) ON DELETE SET NULL,
ADD CONSTRAINT IF NOT EXISTS fk_members_position_status 
...
-- 8. 기존 교회들에 대해 기본 데이터 삽입
-- 실행 전에 실제 church_id를 확인하고 실행하세요
-- SELECT church_id FROM churches; 로 church_id 확인 후 아래 함수 실행
